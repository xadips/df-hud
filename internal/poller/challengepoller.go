...
func NewChallenge(client *Client, creds *CredentialStore, game *GameWatcher, gate *Gate,
...
func (p *ChallengePoller) SetOnBoard(fn func([]model.Challenge)) {
	p.mu.Lock()
	defer p.mu.Unlock()
	p.onBoard = fn
}
...
func (p *ChallengePoller) pauseReason() string {
	...
	if cfg.SigningSalt(p.creds.Salt) == "" {
...
// PauseReason explains why challenge polling is currently suspended.
func (p *ChallengePoller) PauseReason() string { return p.pauseReason() }
...
func (p *ChallengePoller) pollOnce(ctx context.Context) error {
	...
	cfg := p.cfg()
	salt := cfg.SigningSalt(p.creds.Salt)
