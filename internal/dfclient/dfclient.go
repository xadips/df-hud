...
// Credentials is the secret-bearing request identity shared with the
// credential store.
...
// ParseFlash decodes the game's unescaped key/value wire format.
func ParseFlash(body string) (map[string]string, error) { return parseFlash(body) }
...
// LooksLikeHTML reports whether a response is an HTML interstitial rather than
// game data.
func LooksLikeHTML(body string) bool { return looksLikeHTML(body) }
...
// DisablePublicGetValues forces the authenticated fallback. It is primarily a
// deterministic test seam for poller request-shape tests.
func (c *Client) DisablePublicGetValues() { c.publicFailed.Store(true) }
...
func (c *Client) call(ctx context.Context, endpoint string, p orderedParams, hashed bool, salt string) (map[string]string, error) {
	if !allowedEndpoints[endpoint] {
		return nil, fmt.Errorf("df: endpoint %q is not on the allowlist (never callable: %s)",
			endpoint, strings.Join(forbiddenEndpoints, ", "))
	}
	if hashed && salt == "" {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost,
		c.BaseURL+"/"+endpoint+".php", strings.NewReader(body))
	if err != nil {
		return nil, err
	}
