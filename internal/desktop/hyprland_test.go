...
func TestFindGameWindowSkipsTheLauncher(t *testing.T) {
	...
	match := windowMatch{Class: "DeadFrontier.exe", IgnoreTitles: []string{"configuration"}}
...
func TestHyprSocketPath(t *testing.T) {
	...
	if _, err := hyprSocketPath(".socket2.sock"); err == nil {
		t.Error("with no compositor and no signature this must fail")
	}
	...
	if got, err := hyprSocketPath(".socket2.sock"); err != nil || got != want {
		t.Errorf("with a stale signature: (%q, %v), want %q", got, err, want)
	}
