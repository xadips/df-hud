...
type desktopClient interface {
	GameWindow(context.Context, int, windowMatch) (windowPlacement, error)
...
// Placement is the desktop's answer about the game's top-level window.
type Placement = windowPlacement

// Match describes how to identify the game and exclude launcher dialogs.
type Match = windowMatch

// Window is the portable diagnostic shape for a top-level window.
type Window = desktopWindow

// Client is the platform desktop integration used by the command wiring.
type Client = desktopClient
