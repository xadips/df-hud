...
// Store is the concurrency-safe credential holder used by the bridge and
// pollers.
type Store = credStore

// NewStore creates a credential store backed by path. An empty path keeps
// credentials in memory, which is useful to callers that do not want
// persistence.
func NewStore(path string) *Store { return newCredStore(path) }

// Path returns the persistence path.
func (s *credStore) Path() string { return s.path }
