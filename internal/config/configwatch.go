...
// Reload reads path and preserves fields that require a restart.
func Reload(path string, current *Config) (*Config, []string, error) {
	return reloadFrom(path, current)
}
...
// Watcher observes atomic editor saves of a config file.
type Watcher = configWatcher
...
// NewWatcher synchronously registers a directory watch.
func NewWatcher(path string) (*Watcher, error) { return newConfigWatcher(path) }
