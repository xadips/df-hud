...
func newOnslaughtRowWidget() *onslaughtRowWidget {
	...
	rw.label.AddCSSClass(render.OnslaughtLabelClass)
...
var onslaughtContentClasses = []string{
	render.OnslaughtPrevClass,
	render.OnslaughtNowClass,
	render.OnslaughtNextClass,
	render.OnslaughtEmptyClass,
}
...
func (w *bossesWidget) Update(v *View) {
	attack, showAttack := render.OutpostAttackLine(v)
	timer, showTimer := render.OnslaughtHeaderTimer(v)
	onslaughtRows, inOnslaught := render.OnslaughtPanel(v)
	nearest, showNearest := render.NearestLine(v, w.cfg)
