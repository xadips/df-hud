...
func newMapWidget(cfg MapWidgetConfig) *mapWidget {
	...
	bw, bh := render.MapWindowSize(cfg)
	w.frame.Window = render.MapWindow{X: theCity.OriginX, Y: theCity.OriginY, W: bw, H: bh}
...
func (w *mapWidget) cell() int { return render.MapCellPx(w.cfg) }
...
func (w *mapWidget) NaturalSize() (int, int) {
	cell := w.cell()
	bw, bh := render.MapWindowSize(w.cfg)
...
func (w *mapWidget) Update(v *View) {
	w.frame = render.MapFrameFor(v, w.cfg)
	...
	if w.cfg.ShowList {
		w.list.SetMarkup(render.MapListMarkup(v, w.cfg))
	}
	w.area.QueueDraw()
}
...
func (w *mapWidget) draw(_ *gtk.DrawingArea, cr *cairo.Context, _, _ int) {
	...
	for _, o := range outposts {
		w.drawMarker(cr, layout, font, cell, o.X, o.Y, render.OutpostLetter(o.Name), 0.75, 1, 0.75)
	}
	for _, m := range w.frame.Marks {
		...
		if render.CityMarkRinged(m) {
			r, g, b := render.CityMarkInk(m).Floats()
