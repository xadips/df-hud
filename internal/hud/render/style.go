...
// GroupClass namespaces a widget group's CSS class.
func GroupClass(name string) string { return groupClass(name) }

// WidgetSignature identifies the configuration used to build the widget tree.
func WidgetSignature(cfg *Config) string { return widgetSignature(cfg) }

// StyleSheet returns the complete built-in HUD stylesheet.
func StyleSheet(cfg *Config) string { return styleSheet(cfg) }
