...
use crate::win32::{last_err, wide, CLASS_NAME};
...
type ChoosePixelFormatArb = unsafe extern "system" fn(
    HDC,
    *const i32,
    *const f32,
    u32,
...
pub struct GlSurface {
    ...
    pub hdc: HDC,
    rc: HGLRC,
    opengl32: windows_sys::Win32::Foundation::HMODULE,
    #[allow(dead_code)]
...
impl GlSurface {
    pub fn create(
        ...
        if let Some(set_interval) = swap_interval_fn {
            unsafe { set_interval(swap_interval) };
        } else {
        ...
        Ok(Self {
            ...
            rc,
            opengl32,
            swap_interval: swap_interval_fn,
        })
    }
    ...
    pub fn load_proc(&self, name: &str) -> *const c_void {
        ...
        unsafe {
            ...
            match GetProcAddress(self.opengl32, c.as_ptr() as _) {
    ...
    pub fn set_swap_interval(&self, interval: i32) {
        if let Some(set_interval) = self.swap_interval {
            unsafe { set_interval(interval) };
        }
    }
}
...
> {
    ...
    let opengl32 = unsafe { LoadLibraryA(c"opengl32.dll".as_ptr() as _) };
    if opengl32.is_null() {
        unsafe {
            wglMakeCurrent(ptr::null_mut(), ptr::null_mut());
            wglDeleteContext(rc);
        ...
        return Err(last_err("LoadLibraryA opengl32.dll"));
    }
