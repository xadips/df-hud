...
func testPoller(t *testing.T, f *fakeDF, tune func(*Config)) (*Poller, *credStore, *GameWatcher) {
	t.Helper()
	cfg := defaultConfig()
	cfg.Poll.ActiveInterval = duration{20 * time.Millisecond}
	cfg.Poll.IdleInterval = duration{20 * time.Millisecond}
	cfg.Poll.BackoffMax = duration{40 * time.Millisecond}
	cfg.Poll.Jitter = 0
	cfg.Poll.OnlyWhenGameRunning = false
	cfg.DF.Timeout = duration{2 * time.Second}
	if tune != nil {
		tune(cfg)
	}

	store := newCredStore("")
	if _, err := store.Set(Credentials{UserID: "1234567", Password: "hash", SC: "sc"}, "salt"); err != nil {
		t.Fatal(err)
	}
...
func TestPollerFirstPollIsImmediate(t *testing.T) {
	...
	p, _, _ := testPoller(t, f, func(c *Config) {
		c.Poll.ActiveInterval = duration{time.Hour}
		c.Poll.IdleInterval = duration{time.Hour}
	})
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
func TestPollerNeverBurstsUnderWakeStorm(t *testing.T) {
	f := newFakeDF(t)
	p, _, _ := testPoller(t, f, func(c *Config) {
		c.Poll.ActiveInterval = duration{time.Hour} // only wakes can drive polls
		c.Poll.IdleInterval = duration{time.Hour}
	})
...
func TestPollerStopsDeadOnRejectedCredentials(t *testing.T) {
	...
	f.setReply(nil)
	if _, err := store.Set(Credentials{UserID: "1234567", Password: "hash2", SC: "sc2"}, "salt"); err != nil {
		t.Fatal(err)
	}
	p.Resume()
...
func TestPollerWaitsForCredentials(t *testing.T) {
	...
	if _, err := empty.Set(Credentials{UserID: "1", Password: "2", SC: "3"}, ""); err != nil {
		t.Fatal(err)
	}
	p.Wake()
...
func TestPollerRespectsOnlyWhenGameRunning(t *testing.T) {
	...
	game.mu.Lock()
	game.state = GameState{Running: true, PID: 4242, StartedAt: time.Now().Add(-time.Minute)}
	game.mu.Unlock()
	p.Wake()
...
func TestPollerBacksOffOnFailure(t *testing.T) {
	...
	p, _, _ := testPoller(t, f, func(c *Config) {
		c.Poll.ActiveInterval = duration{20 * time.Millisecond}
		c.Poll.IdleInterval = duration{20 * time.Millisecond}
		c.Poll.BackoffMax = duration{200 * time.Millisecond}
	})
...
func TestPollerIntervalFollowsGameState(t *testing.T) {
	...
	p, _, game := testPoller(t, f, func(c *Config) {
		c.Poll.ActiveInterval = duration{7 * time.Second}
		c.Poll.IdleInterval = duration{11 * time.Minute}
	})
	...
	game.mu.Lock()
	game.state = GameState{Running: true, PID: 1, StartedAt: time.Now()}
	game.mu.Unlock()
...
func TestPollerJitterStaysInRange(t *testing.T) {
	f := newFakeDF(t)
	p, _, _ := testPoller(t, f, func(c *Config) {
		c.Poll.ActiveInterval = duration{10 * time.Second}
		c.Poll.Jitter = 0.1
	})
	base := 10 * time.Second
	sawSpread := false
...
func TestPollerBackoffIsCapped(t *testing.T) {
	...
	p, _, _ := testPoller(t, f, func(c *Config) {
		c.Poll.ActiveInterval = duration{10 * time.Second}
		c.Poll.IdleInterval = duration{10 * time.Second}
		c.Poll.BackoffMax = duration{2 * time.Minute}
		c.Poll.Jitter = 0
	})
