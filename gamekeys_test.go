...
// keysHarness wires a gameKeys against values a test can move.
...
func newKeysHarness(t *testing.T) *keysHarness {
	...
	cfg.GameKeys.FPSDelay = duration{5 * time.Second}

	h := &keysHarness{cfg: cfg, send: &fakeSender{}}
	h.keys = newGameKeys(
		func() *Config { return h.cfg },
		func() GameState { return h.game },
		func() windowPlacement { return h.place },
		h.send,
	)
	return h
}
...
func (h *keysHarness) running(pid int, address string) {
	h.game = GameState{Running: true, PID: pid}
	h.place = windowPlacement{Known: true, Address: address}
}
...
func TestFPSKeyWaitsOutTheLauncher(t *testing.T) {
	...
	h.game = GameState{Running: true, PID: 42}
	h.place = windowPlacement{LauncherOnly: true}
	for i := 0; i < 60; i++ {
...
func TestFPSKeyHonoursAReloadedDelay(t *testing.T) {
	h := newKeysHarness(t)
	h.cfg.GameKeys.FPSDelay = duration{time.Minute}
	start := time.Unix(1000, 0)
	h.running(42, "0xabc")
	h.tick(start)
	h.tick(start.Add(30 * time.Second))
...
func TestSendKeyAcceptsRealKeyNames(t *testing.T) {
	for _, key := range []string{"y", "Y", "F1", "Return", "XF86Launch9", "space"} {
		if !hyprKeyName.MatchString(key) {
			t.Errorf("%q rejected, but it is a real keysym name", key)
		}
	}
}
...
func (h *keysHarness) launcherUp(pid int, address string) {
	h.game = GameState{Running: true, PID: pid}
	h.place = windowPlacement{LauncherOnly: true, LauncherAddress: address}
}
...
func TestDismissLauncherIsOffByDefault(t *testing.T) {
	if defaultConfig().GameKeys.DismissLauncher {
...
func TestTheConfigSeedsTheTraySwitches(t *testing.T) {
	...
	cfg.GameKeys.FPSDisplay = true
	cfg.GameKeys.DismissLauncher = true
	k := newGameKeys(func() *Config { return cfg },
		func() GameState { return GameState{} },
		func() windowPlacement { return windowPlacement{} },
		&fakeSender{})
...
// ClientInWorld defaults to true wherever the client has not said otherwise, so
// a machine with no Discord bridge is not left unable to send anything.
func TestClientInWorldDefaultsToTrue(t *testing.T) {
	now := time.Unix(1000, 0)
	s := newStore(nil)
	if !s.ClientInWorld(now) {
		t.Fatal("no presence feed at all blocked the keys")
	}
	s.SetGame(GameState{Running: true, PID: 1})
	s.SetPresence(PresenceState{At: now, Loading: true})
	if s.ClientInWorld(now) {
		t.Fatal("a fresh Loading... did not hold the keys back")
	}
	s.SetPresence(PresenceState{At: now, HasPosition: true, X: 1, Y: 2})
	if !s.ClientInWorld(now) {
		t.Fatal("a position was not treated as being in the world")
	}
	// Stale: the client stopped talking, so it is no longer evidence of anything.
	if !s.ClientInWorld(now.Add(presenceMaxAge + time.Minute)) {
		t.Fatal("a stale feed blocked the keys forever")
	}
}
