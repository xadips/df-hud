...
// of scheduled polls that produced no usable sample.
type Tick struct {
	At   time.Time
	Vars map[string]string // nil when Err is set
	Err  error
	// Scheduled is false for a one-off poll (-once, or a manual refresh), so
	// those do not count as ticks for the miss colouring.
	Scheduled bool
}

// PollerStatus is what the HUD needs to explain itself when data is not
// arriving. Every field here exists to answer "why is nothing updating?".
type PollerStatus struct {
	Paused       bool
	PauseReason  string
	Stale        bool // credentials rejected; needs a bridge refresh
	Failures     int
...
func newPoller(client *Client, creds *credStore, game *GameWatcher, cfg func() *Config) *Poller {
	return &Poller{
		client: client,
		creds:  creds,
		game:   game,
