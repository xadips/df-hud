...
func testChallengePoller(t *testing.T, level int) (*ChallengePoller, *credStore, *Config) {
	t.Helper()
	cfg := defaultConfig()
	cfg.Poll.OnlyWhenGameRunning = false

	store := newCredStore("")
	if _, err := store.Set(Credentials{
		UserID: "1", Password: "2", SC: "3", Cookie: "DeadFrontierFairview=x",
	}, "y27bigaOAA1"); err != nil {
		t.Fatal(err)
	}
	game := newGameWatcher("DeadFrontier.exe", time.Hour)
	p := newChallengePoller(&Client{}, store, game, newRateGate(time.Millisecond),
...
func TestChallengePollerPauseReasons(t *testing.T) {
	...
	// the bridge reported.
	noSalt := newCredStore("")
	if _, err := noSalt.Set(Credentials{UserID: "1", Password: "2", SC: "3", Cookie: "c"}, ""); err != nil {
		t.Fatal(err)
	}
	saltless := newChallengePoller(&Client{}, noSalt, p.game, p.gate,
	...
	if _, err := creds2.Set(Credentials{UserID: "1", Password: "2", SC: "3"}, "salt"); err != nil {
		t.Fatal(err)
	}
...
func TestChallengePollerIntervalFollowsGameState(t *testing.T) {
	...
	cfg.Poll.ChallengeInterval = duration{30 * time.Second}
	cfg.Poll.IdleInterval = duration{5 * time.Minute}
	...
	p.game.mu.Lock()
	p.game.state = GameState{Running: true, PID: 1, StartedAt: time.Now()}
	p.game.mu.Unlock()
